...
function Constraint:OnReset()
	--DebugUtils.Log("%s:OnReset() idEnt: %s idConstr: %s broken: %s", self:GetName(), tostring(self.idEnt),
...
function Constraint:Apply()
	--DebugUtils.Log("%s:Apply() idEnt: %s idConstr: %s broken: %s", self:GetName(), tostring(self.idEnt),
	--	tostring(self.idConstr), tostring(self.broken))
	...
	elseif (not self.broken) then
		...
		if (ents[1] and ents[6]) then
			VectorUtils.Copy(ConstraintParams.pivot, self:GetPos())
			if self.Properties.bUseEntityFrame==1 then
				VectorUtils.Copy(ConstraintParams.frame1, ents[1]:GetAngles())
			else
				VectorUtils.Copy(ConstraintParams.frame1, self:GetAngles())
			end
			VectorUtils.Copy(ConstraintParams.frame0, ConstraintParams.frame1)
...
function Constraint:OnLevelLoaded()
	--DebugUtils.Log("%s:OnLevelLoaded() idEnt: %s idConstr: %s broken: %s", self:GetName(), tostring(self.idEnt),
	--	tostring(self.idConstr), tostring(self.broken))
...
function Constraint:Event_ConstraintBroken(sender)
	--DebugUtils.Log("%s:Event_ConstraintBroken() idEnt: %s idConstr: %s broken: %s", self:GetName(), tostring(self.idEnt),
	--	tostring(self.idConstr), tostring(self.broken))
	...
	EntityCommon.BroadcastEvent(self, "ConstraintBroken")
end

-- =============================================================================
function Constraint:OnSave( props )
	--DebugUtils.Log("%s:OnSave() idEnt: %s idConstr: %s broken: %s", self:GetName(), tostring(self.idEnt),
	--	tostring(self.idConstr), tostring(self.broken))
...
function Constraint:OnLoad( props )
	...
	--DebugUtils.Log("%s:OnLoad() idEnt: %s idConstr: %s broken: %s", self:GetName(), tostring(self.idEnt),
	--	tostring(self.idConstr), tostring(self.broken))
end
