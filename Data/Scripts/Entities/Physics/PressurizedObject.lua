...
function PressurizedObject:OnReset()
	...
	local bRigidBody = self.Properties.bRigidBody == true
	...
	local PhysFlags = {}
	PhysFlags.flags =  0
	if (self.Properties.bPushableByPlayers == true) then
...
function PressurizedObject.Server:OnHit(hit)
	...
	local playerOnly = LuaUtils.NumberToBool(self.Properties.bPlayerOnly)
	...
	local pass = true
	if (hit.explosion) then pass = LuaUtils.NumberToBool(vul.bExplosion)
	elseif (hit.type=="collision") then pass = LuaUtils.NumberToBool(vul.bCollision); damage = damage * mult.fCollision
	elseif (hit.type=="bullet") then pass = LuaUtils.NumberToBool(vul.bBullet); hit.damage = damage * mult.fBullet
	elseif (hit.type=="melee") then pass = LuaUtils.NumberToBool(vul.bMelee)
	else pass = LuaUtils.NumberToBool(vul.bOther); end
...
function PressurizedObject:CheckDamage(frameTime)
	...
	if (self.leaks > 0) then
		for i,leak in ipairs(self.leakInfo) do
			...
			local hits = Physics.RayWorldIntersection(self.leakPos, VectorUtils.Scale(self.leakDir, self.damageRange), 2, ent_all, self.id, nil, g_HitTable)
			if (hits > 0) then
...
function PressurizedObject:UpdateLeaks(frameTime)
	...
	for i,v in ipairs(self.leakInfo) do
		self:UpdateLeak(frameTime, v, VectorUtils.Normalize(VectorUtils.Scale(self.gravity, -1)))
	end
end
...
-- Compose entity
-- =============================================================================
EntityCommon.MakeUsable(PressurizedObject)
EntityCommon.MakePickable(PressurizedObject)
EntityCommon.MakeTargetableByAI(PressurizedObject)
EntityCommon.MakeKillable(PressurizedObject)
EntityCommon.AddInteractLargeObjectProperty(PressurizedObject);