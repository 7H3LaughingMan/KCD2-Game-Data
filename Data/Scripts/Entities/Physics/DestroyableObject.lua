...
DestroyableObject =
{
	...
	Properties =
	{
		...
		{
			bRigidBody = true, -- True if rigid body.
			bRigidBodyActive = true, -- If rigid body is originally created (true) OR will be created only on OnActivate (false).
...
{
	bRigidBody = false, -- True if rigid body.
	bRigidBodyActive = true, -- If rigid body is originally created (1) OR will be created only on OnActivate (0).
...
function DestroyableObject:OnLoad(table)
	...
	if (self.dead) then
		if (self.Properties.Physics.bRigidBodyAfterDeath == 1) then
			-- temporarily set bRigidBody to true, because EntityCommon checks it!
			local aux = self.Properties.Physics.bRigidBody
			self.Properties.Physics.bRigidBody = true
...
function DestroyableObject:OnReset()
	...
	if(self.timerShooterId)then
		--DebugUtils.Log("self.timerShooterId: "..self.timerShooterId)
	end
...
function DestroyableObject:Reload()
	...
	if (self.bReloadGeoms == 1) then
		if (not string.IsEmpty(props.object_Model)) then
		...
		if (not string.IsEmpty(props.object_ModelDestroyed)) then
			self:LoadSubObject(1, props.object_ModelDestroyed, props.DestroyedSubObject)
		elseif (not string.IsEmpty(props.DestroyedSubObject)) then
...
function DestroyableObject:Explode()
	...
	if (Properties.object_ModelDestroyed ~="" or Properties.DestroyedSubObject ~="") then
		if (Properties.Physics.bRigidBodyAfterDeath == 1) then
			...
			Properties.Physics.bRigidBody = true
	...
	if( CryAction.IsServer() and LuaUtils.NumberToBool(self.Properties.bExplode)) then
		...
		VectorUtils.Copy(offset, expl.vOffset)

		local explo_dir = g_Vectors.temp_v6
		VectorUtils.Copy(explo_dir, expl.Direction)
	...
	EntityCommon.BroadcastEvent( self,"Explode" )
...
function DestroyableObject:ActivateDelayEffect()
	...
	if(explosion.DelayEffect.bHasDelayEffect==1)then
		if(not self.FXSlot or self.FXSlot==(-1))then
			if(not string.IsEmpty(explosion.DelayEffect.ParticleEffect))then
...
function DestroyableObject.Server:OnHit(hit)
	VectorUtils.Copy(self.LastHit.pos, hit.pos)
	VectorUtils.Copy(self.LastHit.impulse, hit.dir or g_Vectors.v000)
	...
	elseif (hit.explosion) then
		pass = LuaUtils.NumberToBool(vul.bExplosion)
	elseif (hit.type=="collision") then
		pass = LuaUtils.NumberToBool(vul.bCollision)
		damage = damage * mult.fCollision
	elseif (hit.type=="bullet") then
		pass = LuaUtils.NumberToBool(vul.bBullet)
		damage = damage * mult.fBullet
	elseif (hit.type=="melee") then
		pass = LuaUtils.NumberToBool(vul.bMelee)
	else
		pass = LuaUtils.NumberToBool(vul.bOther)
	end

	pass = pass and damage > self.Properties.fDamageTreshold;	-- damage needs to be higher than treshold
	--DebugUtils.Log("%s != %s", tostring(hit.shooterId), tostring(g_localActorId))

	if ( (not System.IsMultiplayer()) and pass and LuaUtils.NumberToBool(self.Properties.bPlayerOnly) and
	...
	if (pass) then
		...
		if (LuaUtils.NumberToBool(self.Properties.bOnlyAllowPlayerToFullyDestroyObject)) then
		...
		if (LuaUtils.NumberToBool(self.Properties.bActivateOnDamage)) then
			self:AwakePhysics(1)
		end
		local explosion=self.Properties.Explosion
		if(explosion.DelayEffect.bHasDelayEffect==1)then
			if(not self.FXSlot or self.FXSlot==(-1))then
				...
				VectorUtils.Copy(g_SignalData.point, self:GetPos())
				AI.FreeSignal(10, "OnExposedToExplosion", self:GetPos(), self.Properties.Explosion.Radius, self.id, g_SignalData)

				local rnd=math.RandomFloat(0,1.5)
				--DebugUtils.Log("Setting Delay on "..self:GetName()..": "..explosion.Delay+rnd)
...
function DestroyableObject:Event_Reset( sender )
	self:OnReset()
	EntityCommon.BroadcastEvent( self,"Reset" )
end
...
function DestroyableObject:Event_Explode( sender )
	...
	EntityCommon.BroadcastEvent( self,"Explode" )
	EntityCommon.BroadcastEvent( self, "Break" )
...
function DestroyableObject:IsUsable(user)
	...
	local ret = nil
	if (self.Properties.bUsable == true and self.bTemporaryUsable == true) then
		ret = 2
	else
		local PhysProps = self.Properties.Physics
		if (PhysProps.bRigidBody == true and PhysProps.bRigidBodyActive == true and user.CanGrabObject) then
...
-- Compose entity
-- =============================================================================
EntityCommon.MakeUsable(DestroyableObject)
EntityCommon.MakePickable(DestroyableObject)
EntityCommon.MakeTargetableByAI(DestroyableObject)
EntityCommon.MakeKillable(DestroyableObject)
EntityCommon.AddInteractLargeObjectProperty(DestroyableObject)
EntityCommon.MakeThrownObjectTargetable(DestroyableObject);