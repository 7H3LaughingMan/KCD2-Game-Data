...
function ProximityTrigger:OnReset()
	...
	if (self.Properties.OnlySelectedEntity~="None" and self.Properties.OnlySelectedEntity~="") then
		...
		if (indexChar and indexChar>1) then
			if (indexChar<5) then
				DebugUtils.LogWarning( "proximity trigger: '%s' is using a too much generic name for 'selectedEntity' field", self:GetName() )
			end
...
function ProximityTrigger:Event_Enter(entityId)
	...
	-- DebugUtils.Log(" ProximityTrigger: %s:Event_Enter(%s) inside: %d", self:GetName(), EntityUtils.GetName(entityId), self.insideCount)
...
function ProximityTrigger.Client:ClEnter(entityId, insideCount)
	-- DebugUtils.Log("ProximityTrigger: %s.Client:ClEnter(%s) count: %d", self:GetName(), EntityUtils.GetName(entityId), insideCount)
...
function ProximityTrigger:Event_Leave(entityId)
	...
	--DebugUtils.Log("%s:Event_Leave(%s)", self:GetName(), EntityUtils.GetName(entityId))
...
function ProximityTrigger.Client:ClLeave(entityId, inside)
	-- DebugUtils.Log("%s.Client:ClLeave(%s, %s)", self:GetName(), EntityUtils.GetName(entityId), tostring(inside))
...
function ProximityTrigger:OnUsed(user)
	...
	DebugUtils.Log("%s:OnUsed(%s)", self:GetName(), EntityUtils.GetName(user))
...
function ProximityTrigger:Trigger(entityId, inside)
	...
	if(self.Properties.ScriptCommand and self.Properties.ScriptCommand~="")then
		_EID = entityId
		local f = loadstring("local entityId=_EID; "..self.Properties.ScriptCommand)
		if (f~=nil) then
			f()
		end
		_EID = nil
