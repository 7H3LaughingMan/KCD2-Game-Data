...
function SpawnGroup:AddSpawnPoints(linkName)
	...
	--DebugUtils.Log("Added %d spawnpoints (linkName %s) for group %s",i,linkName,self:GetName())
end
...
function SpawnGroup:RemoveSpawnPoints(linkName)
	...
	--DebugUtils.Log("Removed %d spawnpoints (linkName %s) for group %s",i,linkName,self:GetName())
end
