Lightning =
{
	...
	{
		bActive = true,
		fDistance = 2000, -- Distance from entity position where strike occurs.
		fDistanceVariation = 0.5, --Variation of the Distance.
		bRelativeToPlayer = 1,
		Timing =
		{
			fDelay = 20,  -- Delay between strikes in seconds
			fDelayVariation = 0.8,  -- Variation of Delay between strikes in percent.
			fLightningDuration = 0.5,  -- in seconds
		},
		Effects =
		{
			LightRadius = 0,
...
function Lightning:_LookupControlIDs()
	...
	self.hRtpcIDInt = AudioUtils.LookupRtpcID(self.Properties.Audio.audioRTPCIntensityRtpc)
end
...
function Lightning:_KillAllAudioTimers()
	...
	while (self.aAudioThunders[nIndex] ~= nil) do
		...
		self.aAudioThunders[nIndex].fIntensity = 0.0
		nIndex = nIndex + 1
	end
end
...
function Lightning:OnInit()
	...
	local oAudioThunderEntry = {nTimerID = self.nAudioTimerMinID, nAudioProxyID = self:GetDefaultAuxAudioProxyID(), fDistance = 0.0, vStrikeOffset = {x = 0.0, y = 0.0, z = 0.0}, fIntensity = 1.0}
	...
	for i = 1, self.nNumAuxAudioProxies do
		oAudioThunderEntry = {nTimerID = self.nAudioTimerMinID + i, nAudioProxyID = self:CreateAuxAudioProxy(), fDistance = 0.0, vStrikeOffset = {x = 0.0, y = 0.0, z = 0.0}, fIntensity = 1.0}
		table.insert(self.aAudioThunders, oAudioThunderEntry)
	end
		...
		while (self.aAudioThunders[nIndex] ~= nil) do
			if (self.aAudioThunders[nIndex].nTimerID == nTimerId) then
				...
				if (self.hOnTriggerID ~= nil) then
					...
					if (self.hRtpcIDInt ~= nil) then
						self:SetAudioRtpcValue(self.hRtpcIDInt, self.aAudioThunders[nIndex].fIntensity, self.aAudioThunders[nIndex].nAudioProxyID)
					end
...
function Lightning:Event_Strike()
	if (self.bStriking == 0) then
		...
		--if (self._StrikeCount == 0) then
			--self.vPrevSkyColor = table.DeepCopy(System.GetSkyColor())
		--end
		...
		if (self.hOnTriggerID ~= nil) then
			...
			oAudioThunder.fIntensity = self.light_intensity
		...
		self:SetTimer( 1,self:GetValueWithVariation(Timing.fLightningDuration,0.2)*1000 )
		self:Activate(1)
	end
	EntityCommon.BroadcastEvent( self,"Strike" )
end
