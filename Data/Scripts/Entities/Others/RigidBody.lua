RigidBody = {
  ...
  MapVisMask = 0,
	...
	Properties = {
		...
		bRigidBodyActive = true, -- If rigid body is originally created OR will be created only on OnActivate.
...
function RigidBody:OnReset()
	...
	local physType
	if (self.Properties.bRigidBodyActive == true) then
		physType = PE_RIGID
...
function RigidBody:Event_OnTouch(sender)
	EntityCommon.BroadcastEvent( self,"OnTouch" )
end
