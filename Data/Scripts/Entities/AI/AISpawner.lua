...
--		self.spawnedEntity = 0
		if(newEntity) then
			...
			if(link.PropertiesInstance.bAutoDisable == false) then
...
--AI.LogEvent(" >>>FindSpawnReinfPoint")
	...
	DebugUtils.Log("FindSpawnReinfPoint")
	...
	while (link) do
		if(AI.GetTypeOf( link.id ) == AIOBJECT_WAYPOINT) then
			local distSq=VectorUtils.DistanceSquared2D( targetPos, link:GetPos() )
	...
	DebugUtils.Log("FindSpawnReinfPoint end")
end

-- =============================================================================
function AISpawner:SpawnInitially()
	DebugUtils.Log("SpawnInitially")
--AI.LogEvent(" >>>Spawning initially : NumUnits is "..self.Properties.NumUnits)
	...
	DebugUtils.Log("SpawnInitially end")
end
...
function AISpawner:CreateDummy()
	...
	DebugUtils.Log("createdummy start")
	self:RemoveDummy()
...
--		ent:SetLocalBBox()
	...
	DebugUtils.Log("createdummy end")
end
...
--	end
--
--	EntityCommon.BroadcastEvent(self, "Spawn")
--end
...
function AISpawner:Event_Limit(params)
	...
	EntityCommon.BroadcastEvent(self, "Limit")
end
...
function AISpawner:Event_AutoDisableOn(params)
	...
	EntityCommon.BroadcastEvent(self, "AutoDisableOn")
end
...
function AISpawner:Event_AutoDisableOff(params)
	...
	EntityCommon.BroadcastEvent(self, "AutoDisableOff")
end
