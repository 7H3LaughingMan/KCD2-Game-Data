...
function RopeEntity:Event_SegCount( sender, nsegs )
	...
	self:SetPhysicParams(PHYSICPARAM_ROPE, RopeParams)
	local dpos = VectorUtils.Negate(self:GetWorldPos())
	local p0 = VectorUtils.Sum(self.Attachment.end1, dpos)
	local p1 = VectorUtils.Sum(self.Attachment.end2, dpos)
	self:SetLocalBBox( { x=math.min(p0.x,p1.x), y=math.min(p0.y,p1.y), z=math.min(p0.z,p1.z) }, { x=math.max(p0.x,p1.x), y=math.max(p0.y,p1.y), z=math.max(p0.z,p1.z) }	)
end
